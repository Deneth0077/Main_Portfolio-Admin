...
export const addNewSoftwareApplication = (data) => async (dispatch) => {
  ...
  try {
    const response = await axios.post(
      "https://portfolio-main-lemon-gamma.vercel.app/api/v1/softwareapplication/add",
      data,
      {
        withCredentials: true,
...
export const deleteSoftwareApplication = (id) => async (dispatch) => {
  ...
  try {
    const response = await axios.delete(
      `https://portfolio-main-lemon-gamma.vercel.app/api/v1/softwareapplication/delete/${id}`,
      {
        withCredentials: true,
      }
    );
